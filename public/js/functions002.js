...
function getElementTop(element_id) {
	if (document.layers) {
		return getObjNN4(document, element_id).pageY;
	}
	let yPos = document.getElementById(element_id).offsetTop;
	let tempEl = document.getElementById(element_id).offsetParent;
	while (tempEl != null) {
...
function toonPopup(url, titleText, width, e) {
	if (e.screenX) {
	...
	document.getElementById('trein').style.width = width+'px';
	document.getElementById('treinTitel').innerHTML = titleText;
