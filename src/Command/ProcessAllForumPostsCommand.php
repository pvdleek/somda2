...
class ProcessAllForumPostsCommand extends Command implements ScheduledJobInterface
{
    /**
     * @var string
     */
    protected static $defaultName = 'app:process-all-forum-posts';
    ...
    {
        $this->setDescription('Process all forum posts');
    }
    ...
    {
        $postIds = $this->doctrine->getRepository(ForumPost::class)->findAllAndGetIds();
        foreach ($postIds as $postId) {
            $post = $this->doctrine->getRepository(ForumPost::class)->find($postId);

            $this->removeAllWordsForPost($post);

            $words = $this->getCleanWordsFromText($post->text->text);
            ...
            if ($postNrInDiscussion === 0) {
                ...
                $titleWords = $this->getCleanWordsFromText($post->discussion->title);
                $this->processWords($titleWords, $post, true);
                $this->doctrine->getManager()->flush();

                $words = array_diff($words, $titleWords);
            }
            $this->processWords($words, $post);
